...
def fetch_chunks_for_document(doc_id: int):
    res = (
        ...
        .select("chunk_index, text_content, metadata, chunk_type, parent_chunk_id")
...
with st.sidebar:
    ...
    page = st.radio(
        "選擇功能",
        ["📊 系統概況", "📤 上傳與匯入", "🗃️ 文件管理", "📖 專有名詞字典", "🔍 檢索測試", "💬 AI 問答", "⚙️ 系統設定"],
    ...
    with sc1:
        if st.button("💾 儲存設定", type="primary", use_container_width=True):
            ...
            if errors:
                ...
                results = []
                results.append(rag_cfg.set("hybrid_threshold",  str(round(new_threshold, 2))))
                results.append(rag_cfg.set("top_k_multiplier",  str(new_top_k_mul)))
                results.append(rag_cfg.set("sim_weight",         str(round(new_sim, 2))))
                results.append(rag_cfg.set("year_weight",        str(round(new_yr, 2))))
                results.append(rag_cfg.set("source_weight",      str(round(new_src, 2))))
                results.append(rag_cfg.set("system_prompt",      new_prompt.strip() if new_prompt.strip() else "{{DEFAULT}}"))
